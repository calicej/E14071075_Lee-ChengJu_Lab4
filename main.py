import pygame
from enemy import EnemyGroup, Enemy
...
class Game:
    ...
    def select(group, x, y):
        ...
        if the item is clicked, then
            the item is selected.n
        else, 
            the item is not selected.
        end if
        end for
        """
        for i in range(len(group.get())):
            group.get()[i].get_selected(group.get()[i].is_clicked(x, y))
            # len(group.get())：共有幾個 tower
            # group.get()[i]：第 i 個 tower
            # group.get()[i].is_clicked(x, y)：滑鼠游標座標(x,y) 有無在 tower[i] 內 (return Bool)
            # group.get()[i].get_selected(Bool)：根據 Bool，改變 tower[i] 灰色圈圈有無顯示之狀態
